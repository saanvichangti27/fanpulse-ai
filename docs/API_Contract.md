...
### Version 3.0.1 — FROZEN
*(3.0.1: added `twitter_sim` Source — labeled simulated tweet stream as an additional demo source.)*
...
- **Source:** `reddit | youtube | news | replay | twitter_sim`
  > `twitter_sim` is an explicitly-labeled SIMULATED tweet stream (X removed its free API tier). Generated by `backend/ingestion/twitter_sim.py` into a replay file; the text is synthetic but flows through the same real NLP/analytics path. Never present it as real Twitter data.
